...
export function useOrderSearchFilter(searchQuery: string) {
  ...
  const filter = useMemo(() => {
    if (!debouncedQuery.trim()) return undefined;
    // Search in orderNumber, email, name, and phone
    return `orderNumber match "*${debouncedQuery}*" || email match "*${debouncedQuery}*" || address.name match "*${debouncedQuery}*" || address.phone match "*${debouncedQuery}*"`;
  }, [debouncedQuery]);
...
export function useContactSearchFilter(searchQuery: string) {
  ...
  const filter = useMemo(() => {
    if (!debouncedQuery.trim()) return undefined;
    return `name match "*${debouncedQuery}*" || email match "*${debouncedQuery}*" || subject match "*${debouncedQuery}*" || message match "*${debouncedQuery}*"`;
  }, [debouncedQuery]);
