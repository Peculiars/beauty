...
interface OrderHandle {
  documentId: string;
}

function OrderRow({ documentId }: OrderHandle) {
  const [data, setData] = useState<OrderProjection | null>(null);

  useEffect(() => {
    async function fetchData() {
      const doc = await client.fetch<OrderProjection>(
        `*[_id == $id][0]{
          _id,
          orderNumber,
          email,
  ...
  return (
    <Link
      href={`/administrator/orders/${documentId}`}
...
function RecentOrdersContent() {
  ...
  useEffect(() => {
    async function fetchOrders() {
      const result = await client.fetch<OrderProjection[]>(
  ...
  return (
    <div className="space-y-2">
      {orders.map((order) => (
        <Suspense key={order._id} fallback={<OrderRowSkeleton />}>
          <OrderRow documentId={order._id} />
        </Suspense>
      ))}
    </div>
  );
