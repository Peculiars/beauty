...
function ProductRowContent({ documentId }: ProductRowProps) {
  const [data, setData] = useState<ProductProjection | null>(null);
  const [document, setDocument] = useState<any>(null);

  useEffect(() => {
    async function fetchData() {
      const projectionQuery = `*[_id == $id][0]{
        name,
        "slug": slug.current,
        stock,
        price,
      ...
      const [projData, fullDoc] = await Promise.all([
        client.fetch<ProductProjection>(projectionQuery, { id: documentId }),
        client.fetch(docQuery, { id: documentId }),
      ]);
  ...
  return (
    <TableRow className="group">
      ...
      <TableCell className="py-3 sm:py-4">
        <Link
          href={`/administrator/inventory/${documentId}`}
      ...
      <TableCell className="hidden py-4 md:table-cell">
        <Suspense fallback={<Skeleton className="h-8 w-24" />}>
          <PriceInput documentId={documentId} />
        </Suspense>
      </TableCell>
      ...
      <TableCell className="hidden py-4 md:table-cell">
        <div className="flex items-center gap-2">
          <Suspense fallback={<Skeleton className="h-8 w-20" />}>
            <StockInput documentType="product" documentId={documentId} />
          </Suspense>
      ...
      <TableCell className="hidden py-4 lg:table-cell">
        <Suspense fallback={<Skeleton className="h-8 w-8" />}>
          <FeaturedToggle documentId={documentId} />
        </Suspense>
      </TableCell>
      ...
      <TableCell className="hidden py-4 sm:table-cell">
        <div className="flex items-center justify-end gap-2">
          <Suspense fallback={null}>
            <RevertButton documentType="product" documentId={documentId} size="sm" />
          </Suspense>
          <Suspense fallback={null}>
            <PublishButton documentType="product" documentId={documentId} size="sm" variant="outline" />
          </Suspense>
        </div>
      </TableCell>
    </TableRow>
