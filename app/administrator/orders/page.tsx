...
function OrderListContent({ statusFilter, searchFilter }: OrderListContentProps) {
  ...
  const fetchOrders = async (offset = 0) => {
    ...
    try {
      ...
      const fetched: OrderDocument[] = await client.fetch(query);

      setOrders((prev) => (offset === 0 ? fetched : [...prev, ...fetched]));
      setHasMore(fetched.length === batchSize);
    } catch (err) {
      console.error(err);
    } finally {
      setIsLoading(false);
    ...
    <>
      <div className="overflow-hidden rounded-xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900">
        <Table>
          ...
          <TableBody>
            {orders.map((order) => (
              <OrderRow key={order._id} documentId={order._id} />
            ))}
          </TableBody>
        </Table>
      </div>
