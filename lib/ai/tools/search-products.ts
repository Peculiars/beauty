...
import { COLOR_VALUES, MATERIALS_SANITY_LIST } from "@/lib/constants/filters";
...
const productSearchSchema = z.object({
  query: z
    ...
    .default("")
    .describe(
      "Search term to find products by name, description, or category (e.g., 'sneakers', 'leather boots', 'men's shoes')"
    ),
  category: z
    .string()
    .optional()
    .default("")
    .describe(
      "Filter by category slug (e.g., 'men', 'women')"
    ),
  material: z
    .enum(["", ...MATERIALS_SANITY_LIST.map((m) => m.value)])
    .optional()
    .default("")
  ...
  minPrice: z
    ...
    .optional()
    .default(0)
    .describe("Minimum price in NGN (e.g., 100)"),
  maxPrice: z
    .number()
    .optional()
    .default(0)
...
export const searchProductsTool = tool({
  description:
    "Search for shoes in the fashion store. Can search by name, description, or category, and filter by material, color, and price range. Returns product details including stock availability.",
